...
test("full gig lifecycle: post, bid, accept, fund, deliver, release", async ({ page }) => {
  ...
  await page.route(`**/api/v1/jobs/${mockJobId}`, async (route) => {
    await route.fulfill({
      ...
      body: JSON.stringify({
        ...
        freelancer_address: jobStatus !== "open" ? "GD...FREELANCER" : undefined,
  ...
  // Accept bid — this was missing, causing the real backend to be hit and the
  // redirect to /fund to never happen because the request threw in catch.
  await page.route(`**/api/v1/jobs/${mockJobId}/bids/${mockBidId}/accept`, async (route) => {
  ...
  await expect(
    page.getByRole("heading", { name: "Build a Soroban Smart Contract" }).first()
  ).toBeVisible();
  ...
  await expect(
    page.getByRole("paragraph").filter({ hasText: "I have extensive experience with Soroban and Rust" })
  ).toBeVisible();

  // 3. Client accepts the bid — now routed to the mock above, so redirect fires
  ...
  // Mark job as funded so the success state renders
  jobStatus = "funded";
