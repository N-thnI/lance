...
async fn index_next_ledgers(pool: &PgPool, client: &Client, rpc_url: &str) -> Result<bool> {
    ...
    let mut last_ledger: i64 = sqlx::query_scalar("SELECT last_processed_ledger FROM indexer_state WHERE id = 1")
        .fetch_optional(pool)
        .await?
        .unwrap_or(0);
    ...
    let resp_val: serde_json::Value = client.post(rpc_url)
        .json(&events_req)
        .send()
        .await?
        .error_for_status()?
    ...
    let result = resp_val.get("result").ok_or_else(|| anyhow!("No result in RPC response"))?;
    ...
    if let Some(events_list) = events {
        for event in events_list {
            let ledger = event.get("ledger").and_then(|v| v.as_str()).and_then(|v| v.parse::<i64>().ok()).unwrap_or(start_ledger);
            let evt_id = event.get("id").and_then(|v| v.as_str()).unwrap_or("");
            let contract_id = event.get("contractId").and_then(|v| v.as_str()).unwrap_or("");
            let topic_hash = event.get("topic").and_then(|arr| arr.as_array()).and_then(|a| a.get(0)).and_then(|v| v.as_str()).unwrap_or("");
            ...
            if inserted.rows_affected() > 0 {
                metrics().total_events_processed.fetch_add(1, Ordering::Relaxed);
                processed_any = true;
            }
        }
    }
    ...
    // 4. Update checkpoint
    let latest_ledger = result.get("latestLedger").and_then(|v| v.as_i64()).unwrap_or(start_ledger);
    let next_checkpoint = std::cmp::max(start_ledger, latest_ledger);

    sqlx::query("UPDATE indexer_state SET last_processed_ledger = $1, updated_at = NOW() WHERE id = 1")
    ...
    metrics().last_processed_ledger.store(next_checkpoint, Ordering::Relaxed);
...
async fn get_latest_ledger(client: &Client, rpc_url: &str) -> Result<i64> {
    ...
    let sequence = resp_val.get("result")
